...
Similar to "Fun2DSine", except that two periods (instead of one) of sine/cosine are fitted.
Then, compare with radial basis interpolation/extrapolation

Step 1: Compile and run datagen.cpp
  $g++ datagen.cpp
  $./a.out
  ==> Will generate two txt files containing the training and validation data sets.
  ==> Default: 4000 sample points for training, 800 points for validation
...
Step 5: Run RBF interpolation/extrapolation
  [Compile the "radial" code by cmake and make.]
  $python run_rbf.py
  ==> Edit run_rbf.py to change RBF parameters.
  ==> Will generate two plots, showing interpolation & extrapolation results.
